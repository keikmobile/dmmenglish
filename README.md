...
**ブックマークレット**、**Chrome拡張**、**スマホ用リダイレクト**、**macOS起動スクリプト**、**ダッシュボード**の5種類を収録。
...
## 5. ダッシュボード（`dashboard.html`）

DMM英会話・NativeCamp の受講履歴を可視化する統合ダッシュボード。GitHub Pages で公開しており、ブラウザから直接開ける。

**URL**: `https://keikmobile.github.io/dmmenglish/dashboard.html`

### 機能

| 機能 | 説明 |
|------|------|
| 今すぐ予約 | DMM英会話（`bookmarklet.js` と同一ロジック）・NativeCamp への即時遷移ボタン |
| JSONインポート | DMM Lesson Tracker・NC Lesson Tracker のエクスポートファイルをドロップで読み込み |
| サマリー | 総レッスン数 / 累計受講時間 / 月平均 / 今月 / 最多講師 / 受講月数 |
| 講師 TOP 10 | DMM(赤) / NC(青) で色分けしたバーチャート |
| 月別推移 | 直近18ヶ月。DMM(赤) / NC(青) / 混在(金) で色分け |

> データはメモリのみ保持（`localStorage` 不使用）

---

## 6. UI要素マスク ブックマークレット（補助ツール）

Chrome拡張を使わない場合や、セレクタ調査に使うブックマークレット2本。

### 6-1. 要素ピッカー（`bookmarklet-picker.js`）
